...
bool mceliece_keygen(PublicKey** public_key, PrivateKey** private_key) {
    ...
    PrivateKey* sk = malloc(sizeof(PrivateKey));     
    ...
    generate_s(sk->s);
    generate_p(sk->p);
    memcpy(sk->g, g, MATRIX_G_ROWS);
    memcpy(sk->ht, h, MATRIX_H_ROWS);
    ...
    mult_matrices(sk->s, MATRIX_S_DIM, MATRIX_S_DIM, sk->g, MATRIX_G_COLS, sg);
    mult_matrices(sg, MATRIX_SG_ROWS, MATRIX_SG_COLS, sk->p, MATRIX_P_DIM, pk->sgp);
...
bool mceliece_decrypt(const PrivateKey* private_key, const uint8_t* in,
            const size_t in_len, uint8_t** out, size_t* out_len) {
