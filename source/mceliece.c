...
static void keygen(PublicKey* publickey, PrivateKey* secretkey) {
    const uint8_t g = { 0 };
    ...
    mult_matrices(secretkey->s, MATRIX_S_DIM, MATRIX_S_DIM, 
        secretkey->g, MATRIX_G_COLS, sg);
    mult_matrices(sg, MATRIX_SG_ROWS, MATRIX_SG_COLS, 
        secretkey->p, MATRIX_P_DIM, publickey->sgp);
}
